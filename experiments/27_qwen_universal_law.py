...
STRENGTHS = [0.01, 0.1] # Micro-Smooth and Mild-Smooth
...
def main():
    print(f"Loading {MODEL_NAME} for Universal Law Check...")
    tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)
    model = AutoModelForCausalLM.from_pretrained(MODEL_NAME, torch_dtype=torch.float16, device_map="auto")
    
    num_layers = model.config.num_hidden_layers
    print(f"Model has {num_layers} layers. Targeting last 3 layers.")
    
    # Target last 3 layers
    target_layers = [num_layers-3, num_layers-2, num_layers-1]
    ...
    for layer in target_layers:
        for strength in STRENGTHS:
            ...
            # Save Raw Results
            with open("output/qwen_universal_law.json", 'w') as f:
                json.dump(results, f, indent=2)

    print("\nUniversal Law Check Complete.")
