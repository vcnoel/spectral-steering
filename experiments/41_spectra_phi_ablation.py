...
CONFIGS = {
    "Baseline": {},
    "Spectra_Phi_V1": {8: 0.3, 15: -0.2, 24: 0.3},
    "Heavy_Shield":   {8: 0.3, 9: 0.3, 15: -0.2, 24: 0.3},
    "Core_Hybrid":    {8: 0.3, 9: 0.3, 15: -0.2},
    "L15_Only":       {15: -0.2}
}

OUTPUT_FILE = "output/spectra_phi_ablation_results.json"
...
# --- Steering Logic (SVD) ---
def apply_steering(model, config):
    if not config:
        return
...
def main():
    print("=== SPECTRA-PHI ABLATION STUDY ===")
    ...
    # Backup ALL MLP weights to be safe/lazy, or just the ones we touch.
    # We touch 8, 9, 15, 24.
    TOUCHED_LAYERS = [8, 9, 15, 24]
    original_weights = {}
    print("Backing up weights...")
    for idx in TOUCHED_LAYERS:
    ...
    for name, config in CONFIGS.items():
        ...
        results[name] = {"math": math_acc, "sycophancy": syc_rate, "config": config}
